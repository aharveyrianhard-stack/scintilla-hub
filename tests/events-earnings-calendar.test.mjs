...
test("the calendar reads earnings_events and writes nothing", () => {
  ...
  /* two read paths - the view's own range with every column, and the tape's much wider range with
     ticker and date only - both through pgErn, so a retired (superseded) date reaches neither. */
  assert.deepEqual([...new Set(reads.map((r) => r.replace(/^pgErn\(|^pg\(/, "").split("?")[0].replace(/"/g, "")))].sort(), ["cohorts", "earnings_events"]);
  assert.match(core, /pgErn\("earnings_events\?"/, "the calendar reads through pgErn, so a superseded date never reaches it");
  assert.match(core, /pgErn\("earnings_events\?select=ticker,date&date=gte\./, "the tape reads two columns, through pgErn too");
...
test("M40 — DAY, WEEK and MONTH are three squares now, and the older list is kept whole", () => {
  ...
  assert.match(src, /\.se-grid\{ display:grid; gap:9px; padding:3px 9px 10px; align-items:stretch;/);
