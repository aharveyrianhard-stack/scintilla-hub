...
7. Run `await xfeedCycle.finish('overlap')` separately. A successful pass publishes once. If publication failed after source completion, retry finishing that same pass; the service retains the source checkpoint and retries publication. Read `/health` and the page result to distinguish completed source collection from successful publication.
