...
**v2 changelog (2026-09-25, decisions D3–D5):** three used-but-unregistered bar flags registered (`bar.exDiv` T-9, `bar.short` V-7, `bar.basis` V-1); three deliberate reservations listed as RESERVED, not registered; the eight parameters that were registered twice now show one owner, the other listing marked as a citation.
...
- `bar.earnings` — bool session flag — T-9 · `bar.exDiv` — bool session flag (ex-dividend open) — T-9 **[v2, D3]**
...
### Reserved names — named in chapter text, deliberately unregistered [v2, D4]
