...
1. **The v1 stack (M-1):** {W CONTEXT, D RECORD} is the minimal honest stack for the calibration class. Add M (monthly) as a second context frame now, or hold at two frames until the alignment tables justify a third? Author leans two — every added frame doubles the conflict surface and the book has no measured alignment lift yet.
2. **FRAME-FIRST (M-3):** the author's ground (frame authority is bar-contract authority; degree is recursion over one frame) is stated and sweepable. If the loop's `mtf.conflictOdds` favors DEGREE-FIRST in some class, the default flips by evidence — confirm the D-8 pattern applies (stated default, data overturns).
3. **WARN scope (M-5):** v1 stamps WARN only from *confirmed* record-frame events (BOS, S-3, range birth). The aggressive alternative — WARN from record-frame *claims* — doubles sensitivity and noise. Author holds confirmed-only; the lead-time table will show whether the earlier warning is worth its false-positive rate.

*Chapter 14 v1 · 8 cards · DRAFT · review_ref: PUBLISHER-REVIEW-R6 (go-ahead). Compiled against: Chapters 1–13. One frame per truth, authority downward, warnings never verdicts, constructions the only way upstairs — and the higher-timeframe doctrine sent to the same table as every other claim. The context pair is complete. — THE SCINTILLA RULEMAKER*
