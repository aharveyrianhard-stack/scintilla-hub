...
- WHY IT WORKS: The loop proved the edge is regime-conditional; X made the condition a state; this card is where a reader's account finally meets that fact — scaled participation in what the book can defend, near-zero in what it cannot, with the ledger recording that the difference was appetite, never truth.
- STATUS: DRAFT
...
1. **The reader-model ruling (R1's open item, now concrete):** trader-only v1 as shipped, or add the INVESTOR row now (fourth profile, longer clocks, W-record stack)? The mechanism is ready either way; author leans trader-only until the calibration tables exist, so the investor row launches with numbers instead of vibes.
2. **The MODERATE doctrine default (K-3):** context-keyed-by-table is the design intent, but the table is UNMEASURED at birth — v1 falls back to ENTRY-R. Confirm the fallback, or rule ENTRY-R for all profiles until `risk.entryDoctrineOdds` fills (the conservative uniform start).
3. **Ladder universality (K-7):** shipped mandatory-for-all-profiles (steepness varies). If any future profile class (e.g., the investor row) wants a shallower or R-free ladder, it still must have *a* ladder — confirm mandatory-existence as law, not default.

*Chapter 16 v1 · 9 cards · DRAFT · review_ref: PUBLISHER-REVIEW-R6 (Sprint 8 go-ahead). Compiled against: Chapters 1–15. The reader enters; the truth doesn't move. Sixteen chapters, one contract, one math file — the full first draft of THE SCINTILLA TRADING RULEBOOK stands complete, every deposit cashed or stated, awaiting the formalization pass. — THE SCINTILLA RULEMAKER*
