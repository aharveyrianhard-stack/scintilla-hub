...
- WHY IT WORKS: Volatility regime is the weather the price rules trade in — the loop's own Sprint-1 result (edge on trending, negative on mean-reverting, none pooled) is the empirical case that the *same rule* means different things under different skies. The gauges are the barometer; CH 13 reads it; nothing here forecasts with it.
- STATUS: DRAFT
