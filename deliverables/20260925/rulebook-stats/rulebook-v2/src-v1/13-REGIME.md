...
- WHY IT WORKS: The machine measured it before the book could assert it: pooled, the chain nets to nothing; conditioned, it has an edge and a sign. A regime column on every table is how that finding becomes permanent furniture instead of a remembered anecdote — and how every future rate the book publishes says *where* it is true.
- STATUS: DRAFT
...
| `vol.earnStand` | STAND · DISCOUNT | X-8 (rules Q-9's default) | STAND default; DISCOUNT sweeps |
| `vol.calStand` | declared-date set | X-8 | named dates or empty — no silent set |
...
1. **TREND-IN-RANGE's counter-trend traps (X-4):** v1 keeps springs/upthrusts against the trend ACTIVE with a state stamp (measure, don't gate). The conservative alternative — SUSPEND counter-trend traps in TREND-IN-RANGE — is one lookup cell away. Author holds: let the table decide; gate only if the state's trap odds come back inverted.
2. **The dead zone's scope (X-7):** v1 applies the minimal core after *every* transition, including range-birth (arguably the gentlest transition). Author leans uniform treatment for two-strangers simplicity; flag if testing shows range-birth dead zones cost more edge than they save.
3. **`regime.lookup` as a swept object:** the loop sweeping lookup cells is powerful and dangerous (many cells, multiple-comparisons risk). Author proposes the audit discipline applies: cells change only on outcome evidence that clears the loop's own gates, never on a single green run. Confirm the bar.

*Chapter 13 v1 · 9 cards · DRAFT · review_ref: PUBLISHER-REVIEW-R6 (go-ahead). Compiled against: Chapters 1–11 + 97. The keystone set: states from the ledger, a lookup argued past A-6's bar, climate as a column, inversions as law, the calendar ruled, and a regime column on every number the book will ever publish. The loop's promotion path is unblocked. — THE SCINTILLA RULEMAKER*
